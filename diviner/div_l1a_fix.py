...
# Ch. 1-2 will not be corrected since they were used in the first place to
# derive the correction.

factors = np.loadtxt(
    os.path.join(__path__[0], "data", "scaling_factors.ascii"), dtype=float
)
...
def correct_noise(data):
    ...
    max_diff = ma.max(
        np.transpose(np.dstack((max_diff1, max_diff2)), (1, 0, 2)), axis=2
    )
