...
def prepare_data(df_in):
    ...
    # df[df == -9999.0] = nan
    if "last_el_cmd" not in df_in.columns:
        df_in.rename(
            columns={"el_cmd": "last_el_cmd", "az_cmd": "last_az_cmd"}, inplace=True
        )
    # this was done with `replace` method before, but this is clearer:
    cols_to_fill = ["last_el_cmd", "last_az_cmd", "moving"]
    for col in cols_to_fill:
        df_in[col].ffill(inplace=True)
    return df_in
...
def define_sdtype(df):
    ...
    df["calib_block_labels"] = nd.label(
        (df.sdtype == 1) | (df.sdtype == 2) | (df.sdtype == 3)
    )[0]
