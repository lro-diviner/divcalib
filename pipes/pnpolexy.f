...
c  PROGRAM: pnpolexy
c
c  PURPOSE: This program calculates a simple radial north polar projection
c from latitude and east longitude
c and creates a new columns spolex and spoley
c the map is oriented in the convrntional manner for north polar projections
c where:
c     180 degrees east longitude is pointing up
c     270 degrees east longitude points to the right
c     0 degrees east longitude points down
c     90 degrees east longitude points to the left
...
c  EXAMPLE:
c
c   pnpolexy lon=surflon lat=surflat <data.mcs_35 > data.mcs_37
c
c*******************************************************************************
...
c check for nodes keyword in command line
       ...
       call adddesline(ndeslines+1,'npolex',
     &       'npolex', cdesheader,
     &       cdesstitle,cdesltitle,
     &       ndeslines)
       ...
       call adddesline(ndeslines+1,'npoley',
     &       'npoley', cdesheader,
     &       cdesstitle,cdesltitle,
     &       ndeslines)
...
10           ios = read5(rdat, ndeslines-2)
          ...
          lat=rdat(latcol)
          lon=rdat(loncol)
          npolex=-((90.d0)-lat)*cos(lon*pi/180.d0+0.5*pi)
          npoley=-((90.d0)-lat)*sin(lon*pi/180.d0+0.5*pi)
          rdat(ndeslines-1)=npolex
          rdat(ndeslines)=npoley
